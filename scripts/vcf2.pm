...
sub parse_metainfo {
	...
	$data =~ s/BCFtools\/csq, see http/BCFtools\/csq see http/; # HACKIHACK (remove ,)
...
sub parse_variant {
    ...
    if( $var{ INFO }->{ BCSQ } ) {
	$var{ INFO }->{ BCSQ } = parse_VEP_CSQ( $var{INFO}->{BCSQ}, $meta->{INFO}->{BCSQ} );
    }    
